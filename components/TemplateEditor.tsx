...
interface TemplateEditorProps {
  ...
  onSaveTemplate: (name: string, category: TemplateCategory) => void;
...
export const TemplateEditor: React.FC<TemplateEditorProps> = ({
  ...
  savedTemplates, onSaveTemplate, onDeleteTemplate, onArchiveTemplate
}) => {
  ...
  const handleDrivePickerOpen = async () => {
    const token = sessionStorage.getItem('gmail_access_token');
    if (!token) {
      onToast('Necesitas autorizar Gmail/Drive primero.', 'error');
      return;
    }
    ...
    try {
      ...
      if (file) {
        ...
        onToast(`"${file.name}" adjuntado desde Drive`, 'success');
      }
    } catch (err: any) {
      onToast(err.message || 'Error al abrir Google Drive', 'error');
    } finally {
  ...
  const confirmSave = () => {
    ...
    onSaveTemplate(saveName, saveCategory);
    setSaveName('');
  ...
  // Filtered templates based on search + category + archive state
  ...
  }, [savedTemplates, templateSearch, categoryFilter, showArchived]);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
      <div className="lg:col-span-2 space-y-4">
        ...
        <div className="bg-white dark:bg-dark-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
          ...
          {showSaveDialog && (
            <div className="mb-4 p-4 bg-slate-50 dark:bg-dark-900/50 border border-slate-200 dark:border-slate-700 rounded-lg space-y-3 animate-in fade-in duration-200">
        ...
        <div className="bg-white dark:bg-dark-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
          ...
          >
            ...
            <div className="flex flex-col items-center gap-2">
              <label className="cursor-pointer px-4 py-2 bg-white dark:bg-dark-800 text-slate-700 dark:text-slate-200 rounded-lg text-sm font-medium transition-colors border border-slate-300 dark:border-slate-600 hover:border-indigo-400 hover:text-indigo-600 dark:hover:text-indigo-400 shadow-sm">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-dark-800 text-slate-700 dark:text-slate-200 rounded-lg text-sm font-medium transition-colors border border-slate-300 dark:border-slate-600 hover:border-green-400 hover:text-green-600 dark:hover:text-green-400 shadow-sm disabled:opacity-60"
              >
                {loadingDrive ? <Loader2 className="w-4 h-4 animate-spin" /> : <HardDrive className="w-4 h-4" />}
                {loadingDrive ? 'Cargando...' : 'Añadir desde Drive'}
              </button>
      ...
      <div className="lg:col-span-1 space-y-6">
        ...
        <div className="bg-white dark:bg-dark-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col max-h-[420px]">
          {/* Header */}
          <div className="p-4 border-b border-slate-200 dark:border-slate-700">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-slate-800 dark:text-white flex items-center gap-2">
                ...
                Mis Plantillas
                <span className="text-[10px] bg-slate-100 dark:bg-dark-700 text-slate-500 px-1.5 py-0.5 rounded-full">
                  {savedTemplates.filter(t => !t.archived).length}
                </span>
              </h3>
              <button
                onClick={() => setShowArchived(!showArchived)}
                title={showArchived ? 'Ver activas' : 'Ver archivadas'}
                className={`p-1.5 rounded-lg text-xs transition-colors flex items-center gap-1 ${showArchived ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
              >
                {showArchived ? <ArchiveRestore className="w-3.5 h-3.5" /> : <Archive className="w-3.5 h-3.5" />}
                {showArchived ? 'Activas' : 'Archivo'}
              </button>
            </div>
            ...
            <div className="relative mb-2">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-1.5 bg-slate-50 dark:bg-dark-900/50 border border-slate-200 dark:border-slate-600 text-slate-800 dark:text-slate-200 rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500"
              />
            </div>
            ...
            <div className="flex flex-wrap gap-1">
              <button
                ...
                className={`text-[10px] px-2 py-0.5 rounded-full font-medium transition-colors border ${categoryFilter === 'all' ? 'bg-indigo-600 text-white border-indigo-600' : 'text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-600 hover:border-indigo-400'}`}
              >
                Todas
              </button>
              {TEMPLATE_CATEGORIES.map(cat => (
                <button
                  ...
                  className={`text-[10px] px-2 py-0.5 rounded-full font-medium transition-colors border ${categoryFilter === cat ? 'bg-indigo-600 text-white border-indigo-600' : 'text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-600 hover:border-indigo-400'}`}
                >
                  {cat}
                </button>
              ))}
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-2">
            {filteredSavedTemplates.length === 0 ? (
              <div className="text-center py-6 text-xs text-slate-400 dark:text-slate-500">
                {showArchived ? 'No hay plantillas archivadas.' : templateSearch ? 'Sin resultados.' : 'No hay plantillas guardadas.'}
              </div>
            ) : (
              <ul className="space-y-1">
                {filteredSavedTemplates.map(t => (
                  <li key={t.id} className="flex items-center gap-1 group p-2 hover:bg-slate-50 dark:hover:bg-dark-700 rounded-lg transition-colors border border-transparent hover:border-slate-100 dark:hover:border-slate-700">
                    <div className="flex-1 min-w-0 cursor-pointer" onClick={() => !t.archived && loadTemplate(t)}>
                      <div className="flex items-center gap-1.5">
                        <p className={`text-xs font-medium truncate ${t.archived ? 'text-slate-400 dark:text-slate-500 line-through' : 'text-slate-700 dark:text-slate-200'}`}>
                          {t.name}
                        </p>
                        {t.category && (
                          <span className="text-[9px] bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-800 px-1 py-0.5 rounded-full flex-shrink-0">
                            {t.category}
                          </span>
                        )}
                      </div>
                      <p className="text-[10px] text-slate-400 dark:text-slate-500 truncate">{t.subject}</p>
                    </div>
                    <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                      ...
                      <button
                        onClick={() => onArchiveTemplate(t.id, !t.archived)}

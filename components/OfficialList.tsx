...
import { Official, Gender, SortOption, FilterCriteria, AbsenceRecord, AbsenceConfig, AbsenceType, CompensatoryHourRecord } from '../types';
import { Edit2, Trash2, Search, Mail, Calendar, Clock, Phone, Fingerprint, LayoutGrid, List, Eraser, Building2, History, AlertCircle } from 'lucide-react';
...
export const OfficialList: React.FC<OfficialListProps> = ({
    officials, absences, compensatoryRecords, onEdit, onDelete, onClearAll, sortOption, onSortChange, initialFilter, onClearFilter, config
}) => {
    const [searchTerm, setSearchTerm] = useState('');
    const [viewMode, setViewMode] = useState<'grid' | 'table'>('grid');
    const [selectedOfficialForHistory, setSelectedOfficialForHistory] = useState<Official | null>(null);

    const handleGmailOpen = (email: string) => {
    ...
    return (
        <div className="space-y-4">
                            ...
                            <div key={official.id} className={`bg-white rounded-2xl shadow-sm border p-6 hover:shadow-md transition-all relative overflow-hidden group ${isExpiringSoon ? 'border-amber-300 bg-amber-50/20' : 'border-slate-200'}`}>
                                ...
                                <div className="flex items-start gap-4 mb-4">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2 mb-1">
                                            <h3 className="font-bold text-slate-900 leading-tight">{official.name}</h3>
                                            {(() => {
                                                const progressiveDays = calculateProgressiveDays(official.entryDate, official.recognizedYears || 0);
                                                const totalAnnualLimit = config.legalHolidayLimit + progressiveDays;
                                                const officialAbsences = absences.filter(a => a.officialId === official.id && a.type === AbsenceType.FeriadoLegal);
                                                const usedDays = officialAbsences.reduce((sum, a) => sum + a.days, 0);
                                                const proportionalDays = calculateProportionalDays(official.entryDate, config.legalHolidayLimit);
                                                const accumulated = proportionalDays + progressiveDays - usedDays;

                                                if (accumulated > (totalAnnualLimit * 2)) {
                                                    return (
                                                        <span className="px-1.5 py-0.5 bg-rose-100 text-rose-600 text-[8px] font-black rounded-md flex items-center gap-1 animate-pulse" title={`Saldo acumulado crítico: ${accumulated.toFixed(1)} días`}>
                                        ...
                                        <div className="flex items-center justify-between gap-2">
                                            ...
                                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                                                <button onClick={() => setSelectedOfficialForHistory(official)} className="p-1.5 bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-100 transition-colors" title="Historial de Ausencias"><History className="w-3.5 h-3.5" /></button>
                                                <button onClick={() => onEdit(official)} className="p-1.5 bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100 transition-colors" title="Editar"><Edit2 className="w-3.5 h-3.5" /></button>
                                                <button onClick={() => onDelete(official.id)} className="p-1.5 bg-rose-50 text-rose-600 rounded-lg hover:bg-rose-100 transition-colors" title="Eliminar"><Trash2 className="w-3.5 h-3.5" /></button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-2 mb-6">
                                    ...
                                    <div className="flex items-center gap-2 text-xs text-slate-500">
                                        ...
                                        <button
                                            onClick={(e) => { e.stopPropagation(); handleGmailOpen(official.email); }}
                                            className="truncate hover:text-indigo-600 hover:underline transition-colors font-medium text-left outline-none"
                                        >
                                            {official.email}
                                        </button>
                                    </div>
                                        ...
                                        <tr key={official.id} className={`hover:bg-slate-50 transition-colors ${isExpiringSoon ? 'bg-amber-50/20' : ''}`}>
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="flex flex-col">
                                                        ...
                                                        {(() => {
                                                            ...
                                                            if (accumulated > (totalAnnualLimit * 2)) {
                                                                return (
                                                                    <span className="text-rose-600 text-[8px] font-black flex items-center gap-1 animate-pulse">
                                                                        <AlertCircle className="w-2.5 h-2.5" /> +2 PERIODOS
                                                                    </span>
                                                                );
                                                            }
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="space-y-0.5">
                                                    <button
                                                        onClick={() => handleGmailOpen(official.email)}
                                                        className="text-xs text-indigo-600 font-medium hover:underline flex items-center gap-1 truncate max-w-[180px]"
                                                    >
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex justify-end gap-1">
                                                    <button onClick={() => setSelectedOfficialForHistory(official)} className="p-1.5 text-slate-400 hover:text-emerald-600 transition-colors rounded-md hover:bg-emerald-50" title="Historial">
                                                        <History className="w-3.5 h-3.5" />
                                                    </button>
                                                    <button onClick={() => onEdit(official)} className="p-1.5 text-slate-400 hover:text-indigo-600 transition-colors rounded-md hover:bg-indigo-50" title="Editar">
                                                        <Edit2 className="w-3.5 h-3.5" />
                                                    </button>
                                                    <button onClick={() => onDelete(official.id)} className="p-1.5 text-slate-400 hover:text-rose-600 transition-colors rounded-md hover:bg-rose-50" title="Eliminar">
            ...
            {selectedOfficialForHistory && (
                <AbsenceHistory
                    official={selectedOfficialForHistory}
                    absences={absences}
                    compensatoryRecords={compensatoryRecords}
                    config={config}
                    onClose={() => setSelectedOfficialForHistory(null)}
                />
            )}
        </div>
    );

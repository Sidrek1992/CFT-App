...
import { Official, Gender, ViewState, FilterCriteria, Campaign } from '../types';
import {
...
const getDayKey = (ts: number) => new Date(ts).toISOString().slice(0, 10); // 'YYYY-MM-DD'
...
const buildTimeline = (campaigns: Campaign[], days: 7 | 30 | 90) => {
    ...
    // Build a map: key → index in entries
...
export const Dashboard: React.FC<DashboardProps> = ({
    ...
    const totalInPeriod = timelineData.reduce((acc, d) => acc + d.count, 0);

    // ─── Campaign engagement ───────────────────────────────────────────────────
    const campaignStats = useMemo(() => {
        return campaigns
            .filter(c => c.logs && c.logs.length > 0)
            .map(c => {
                const sent = c.logs.filter(l => l.status === 'sent').length;
                return { name: c.name, sent, createdAt: c.createdAt };
            })
    ...
    return (
        <div className="space-y-6 animate-fade-in-up">
            ...
            {/* 2. Row 2: Timeline chart + Campaign stats */}
                    ...
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="font-bold text-slate-700 dark:text-slate-300 text-sm flex items-center gap-2">
                            ...
                            <span className="text-xs font-normal text-slate-500 dark:text-slate-400">
                                ({totalInPeriod} en {timelineDays} días)
                            </span>
                        </h3>
                        {/* Period selector */}
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={timelineData} margin={{ top: 5, right: 5, bottom: 0, left: -20 }}>
                                    ...
                                    <RechartsTooltip
                                        ...
                                        formatter={(v) => [`${v} enviados`, '']}
                                    />
                                    <Area type="monotone" dataKey="count" stroke="#6366f1" strokeWidth={2} fill="url(#colorCount)" dot={false} activeDot={{ r: 4, fill: '#6366f1' }} />
                                </AreaChart>
                            </ResponsiveContainer>
                        )}
                    </div>
                ...
                <div className="glass-panel bento-card p-6 flex flex-col">
                    ...
                    {campaignStats.length === 0 ? (
                        <div className="flex-1 flex flex-col items-center justify-center text-slate-400 dark:text-slate-500">
                            <p className="text-sm">Sin campañas registradas</p>
                        </div>
                    ) : (
                        <div className="space-y-3 flex-1 overflow-y-auto pr-1 custom-scrollbar">
                            {campaignStats.map((c, i) => (
                                <div key={c.name} className="flex items-center gap-3">
                                    <span className={`text-xs font-black w-5 text-center flex-shrink-0 ${i === 0 ? 'text-amber-400' : 'text-slate-400'}`}>#{i + 1}</span>
                                    <div className="flex-1 min-w-0">
                                        <p className="text-xs font-semibold text-slate-700 dark:text-slate-200 truncate">{c.name}</p>
                                        <div className="w-full bg-slate-100 dark:bg-dark-900 h-1 rounded-full mt-1 overflow-hidden">
                                            <div
                                                className={`h-full rounded-full ${i === 0 ? 'bg-gradient-to-r from-emerald-500 to-teal-400' : 'bg-slate-400 dark:bg-slate-600'}`}
                                                style={{ width: `${Math.round((c.sent / (campaignStats[0]?.sent || 1)) * 100)}%` }}
                                            />
                                        </div>
                                    </div>
                                    <span className="text-xs font-bold text-slate-500 dark:text-slate-400 flex-shrink-0">{c.sent}</span>
                                </div>
                            ))}
                        </div>
                    )}
            ...
            {/* 3. Row 3: Donut + Top Depts + Health */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {/* Donut Chart */}
                <div className="glass-panel bento-card p-6 flex flex-col relative h-[320px]">
                    <h3 className="font-bold text-slate-700 dark:text-slate-300 text-sm mb-4">Distribución Demográfica</h3>
                    {totalOfficials === 0 ? (
                    ...
                    <div className="mt-2 flex flex-wrap justify-center gap-3">
                        {genderData.map(d => (
                            <div key={d.name} className="flex items-center gap-1.5 text-xs text-slate-600 dark:text-slate-400 font-medium">
                                <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: d.color }}></span>{d.name}
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="glass-panel bento-card p-6 flex flex-col h-[320px]">
                    ...
                    <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                        {topDepartments.length === 0 ? (
                            <div className="h-full flex flex-col items-center justify-center text-slate-500 dark:text-slate-500"><p className="text-sm">No hay datos suficientes</p></div>
                        ) : (
                            <div className="space-y-4">
                                {topDepartments.map(([name, count], index) => (
                                    <div key={name} className="relative cursor-pointer group" onClick={() => onNavigate('database', { type: 'department', value: name === 'Sin Dept.' ? '' : name })}>
                                        ...
                                        <div className="w-full bg-white dark:bg-dark-900/50 h-1.5 rounded-full overflow-hidden border border-slate-100 dark:border-white/5">
                                            <div className={`h-full rounded-full transition-all duration-1000 ease-out ${index === 0 ? 'bg-gradient-to-r from-primary-500 to-indigo-400' : 'bg-slate-600'}`} style={{ width: `${(count / maxDeptCount) * 100}%` }}></div>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                <div className="glass-panel bento-card p-6 flex flex-col h-[320px]">
                    ...
                    {totalOfficials === 0 ? (
                        ...
                        <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                            {missingBossCount > 0 && (
                                <div onClick={() => onNavigate('database', { type: 'missingBoss' })} className="flex items-center gap-3 p-4 bg-orange-500/10 border border-orange-500/20 rounded-2xl cursor-pointer hover:bg-orange-500/20 transition-colors group">
                                    <div className="w-10 h-10 rounded-full bg-orange-500/20 flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform"><AlertTriangle className="w-5 h-5 text-orange-400" /></div>
                                    <div><p className="text-sm font-bold text-slate-900 dark:text-white">{missingBossCount} Sin Jefatura</p><p className="text-xs text-orange-400/80">Requerido para flujos de copia.</p></div>
                                </div>
                            )}
                            {unspecifiedGenderCount > 0 && (
                                <div onClick={() => onNavigate('database', { type: 'missingGender' })} className="flex items-center gap-3 p-4 bg-amber-500/10 border border-amber-500/20 rounded-2xl cursor-pointer hover:bg-amber-500/20 transition-colors group">
                                    <div className="w-10 h-10 rounded-full bg-amber-500/20 flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform"><AlertTriangle className="w-5 h-5 text-amber-400" /></div>
                                    <div><p className="text-sm font-bold text-slate-900 dark:text-white">{unspecifiedGenderCount} Sin Género</p><p className="text-xs text-amber-400/80">Click para auto-detectar.</p></div>
                                </div>
                            )}
                            {invalidEmailCount > 0 && (
                                <div onClick={() => onNavigate('database', { type: 'invalidEmail' })} className="flex items-center gap-3 p-4 bg-red-500/10 border border-red-500/20 rounded-2xl cursor-pointer hover:bg-red-500/20 transition-colors group">
                                    <div className="w-10 h-10 rounded-full bg-red-500/20 flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform"><Mail className="w-5 h-5 text-red-400" /></div>
                ...
                {/* Backup Section */}
                <div className="glass-panel bento-card p-6 md:col-span-2 lg:col-span-3 flex flex-col sm:flex-row items-center justify-between gap-6 border-slate-300 dark:border-slate-700/50 bg-gradient-to-r from-slate-100 dark:from-dark-800/80 to-slate-100 dark:to-dark-900/80">
